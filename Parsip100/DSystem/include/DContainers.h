...
public:
	//Copies content from another Vector to current object
	...
	//Appends content from another vector to current object
	void appendFrom( const DVec& from )
	{
		size_t oldEnd = mSize;
		resize( mSize + from.mSize );

		for (size_t i=0; i < from.mSize; ++i)
			mpData[oldEnd+i] = from.mpData[i];
	}

	void assign(size_t newSize, const T& val)
	{
		resize(newSize);
		for(size_t i=0; i < mSize; i++)
			mpData[i] = val;
	...
	//To be compatible with std::list and vector
	bool empty() const { return (mSize == 0);}

	size_t size() const { return mSize; }
	size_t capacity() const { return mSizeAlloc - mSize; }
	...
	//Erases an item but does not free that item
	//Used for copying data across lists
	void detach( iterator it )
	{
		if NOT( it >= mpData && it < (mpData+mSize) )
			throw std::out_of_range( "Out of bounds !" );
	...
	void remove(size_t idxItem)
	{
		if(idxItem >= 0 && idxItem < mSize)
		{
			iterator it = static_cast<iterator>(mpData + idxItem*sizeof(T));
			erase(it);
		}
	}
	...
	void insert( const_iterator rbegin, const_iterator rend)
	{
		insert( end(), rbegin, rend);
	}

	void insert( iterator itBefore, const_iterator rbegin, const_iterator rend)
	{
		if NOT( itBefore >= mpData && itBefore < (mpData+mSize) )
			throw std::out_of_range( "Out of bounds !" );

		size_t oldsize = mSize;
		if(itBefore == mpData + mSize)
		{
			resize( mSize + rend - rbegin + 1);

			size_t i = 0;
			for(const_iterator it = rbegin; it != rend; it++)
			{
				mpData[i] = *it;
				i++;
			}
	...
	bool isItemIndex(size_t idx) const { return ((idx >= 0)&&(idx < mSize));}
