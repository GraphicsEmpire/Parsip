...
namespace{
    ...
    {
        CSkeleton* lpSkeletFrom = reinterpret_cast<CSkeletonPrimitive*>(const_cast<CBlobNode*>(b))->getSkeleton();
        CSkeletonPrimitive* clonned = reinterpret_cast<CSkeletonPrimitive*>(TheBlobNodeFactoryIndex::Instance().CreateObject(lpSkeletFrom->getType()));
        clonned->copyGenericInfo(b);
        clonned->getSkeleton()->setParamFrom(lpSkeletFrom);
        return clonned;
    }

    const bool registered = TheBlobNodeCloneFactory::Instance().Register(typeid(CSkeletonPrimitive), ClonePrimitive);
}
...
COctree CSkeletonPrimitive::computeOctree()
{
    ...
    if(m_skeleton->getType() == bntPrimRing)
    {
