...
int CMeshNode::add(const vec3f& v, ArrayType dest, int count)
{
	if(count > 3 || count < 0)
		return -1;
...
int CMeshNode::addVertex(const vec3f& v)
{
...
int CMeshNode::addNormal(const vec3f& n)
{
...
int CMeshNode::addTexCoord2(const vec2f& t)
{
...
int CMeshNode::addTexCoord3(const vec3f& t)
{
...
void CMeshNode::addFaceIndex(U32 index)
{
	arrIndices.push_back(index);
}

int CMeshNode::countFaceIndexErrors() const
{
...
vec3f CMeshNode::getAsVec3(int index, ArrayType dest) const
{
	vec3f output;
	if(dest == atVertex)
	{
		...
		output.set(&arrVertices[idxFloat]);
	}
	else if(dest == atNormal)
	{
		U32 idxFloat = index * szUnitVertex;
		assert(idxFloat >= 0 && idxFloat <= arrNormals.size() - 3);
		output.set(&arrNormals[idxFloat]);
	}
	else if(dest == atTexture)
	{
		U32 idxFloat = index * szUnitTexCoord;
		assert(idxFloat >= 0 && idxFloat <= arrTexCoords.size() - szUnitTexCoord);
		output.set(&arrTexCoords[idxFloat]);
	}

	return output;
}

vec3f CMeshNode::getVertex(int idxVertex) const
{
	vec3f output;
...
vec3f CMeshNode::getNormal(int idxVertex) const
{
	vec3f output;
	U32 idxFloatArray = idxVertex * szUnitVertex;
...
vec2f CMeshNode::getTexCoord2(int idxVertex) const
{
	assert(szUnitTexCoord == 2);
...
vec3f CMeshNode::getTexCoord3(int idxVertex) const
{
	assert(szUnitTexCoord == 3);
...
COctree CMeshNode::computeBoundingBox() const
{
	COctree oct;
...
*/
/////////////////////////////////////////////////////////////////////////
CMesh::CMesh(const char* chrFileName)
{
...
void CMesh::addNode(CMeshNode* lpMeshNode)
{
	if(lpMeshNode != NULL)
		m_nodes.push_back(lpMeshNode);
}

CMeshNode*	CMesh::getNode(int idx) const
{
...
int CMesh::getNodeCount() const
{
	return (int)m_nodes.size();
}


void CMesh::addMeshMaterial(CMeshMaterial* lpMaterial)
{
	if(lpMaterial != NULL)
		m_materials.push_back(lpMaterial);
}

CMeshMaterial* CMesh::getMaterial(int idx) const
{
...
CMeshMaterial* CMesh::getMaterial(const DAnsiStr& strName) const
{
	for(size_t i=0; i<m_materials.size(); i++)
	{
...
bool CMesh::load(const char* chrFileName)
{
...
COctree CMesh::computeBoundingBox() const
{
	COctree oct;
	if(this->getNodeCount() == 0)
		return oct;

	oct = m_nodes[0]->computeBoundingBox();
	for(size_t i=1; i<m_nodes.size(); i++)
		oct.csgUnion(m_nodes[i]->computeBoundingBox());
	return oct;
}

bool CMesh::readFileContent(const char* chrFileName, std::vector<DAnsiStr>& content)
{
...
bool CMesh::loadMTL(const char* chrFileName)
{
...
bool CMesh::loadObj(const char* chrFileName)
{
	std::vector<DAnsiStr> content;
	if(!readFileContent(chrFileName, content))
		return false;
	...
	for(size_t i=0; i<content.size(); i++)
	{
		...
		else if((words[0] == "v")&&(aNode)&&(ctWords == 4))
		{
			...
			aNode->add(v, CMeshNode::atVertex, 3);
		...
		else if((words[0] == "vn")&&(aNode)&&(ctWords == 4))
		{
			...
			v.y = static_cast<float>(atof(words[2].ptr()));
			v.z = static_cast<float>(atof(words[3].ptr()));
			aNode->add(v, CMeshNode::atNormal, 3);
		}
...
bool CMesh::loadObjGlobalVertexNormal(const char* chrFileName)
{
	std::vector<DAnsiStr> content;
	if(!readFileContent(chrFileName, content))
		return false;
	...
	CMeshNode* lpGlobalNode = new CMeshNode();
	...
	for(size_t i=0; i<content.size(); i++)
	{
		...
		if((words[0] == "v")&&(lpGlobalNode)&&(ctWords == 4))
		{
			...
			v.y = static_cast<float>(atof(words[2].ptr()));
			v.z = static_cast<float>(atof(words[3].ptr()));
			lpGlobalNode->add(v, CMeshNode::atVertex, 3);
			arrProcessed[i] = true;
		}
		else if((words[0] == "vn")&&(lpGlobalNode)&&(ctWords == 4))
		{
			...
			v.y = static_cast<float>(atof(words[2].ptr()));
			v.z = static_cast<float>(atof(words[3].ptr()));
			lpGlobalNode->add(v, CMeshNode::atNormal, 3);
			arrProcessed[i] = true;
		}
