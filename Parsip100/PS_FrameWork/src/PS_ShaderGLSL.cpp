...
CGLShaderProgram::CGLShaderProgram(const char* pszVertexShader, const char* pszFragmentShader)
{
	init();
...
int	 CGLShaderProgram::compileFromFile(const DAnsiStr& strVertexShaderFP, const DAnsiStr& strFragmentShaderFP)
{
...
int CGLShaderProgram::compileCode(const char* vShaderCode, const char* vFragmentCode)
{
...
void CGLShaderProgram::stop()
{
	glUseProgram(0);
	m_bIsRunning = false;
}

bool CGLShaderProgram::removeAllCppComments(DAnsiStr& strCode)
{
	DAnsiStr strOutput;
...
bool CGLShaderProgram::getAllUniformLocationsFromContext()
{
...
int  CGLShaderProgram::getUniformLocation(const char* chrUniformName)
{
	VARPROP var;
...
bool CGLShaderProgram::setAttribLocation(const char* chrAttribName, int idxLoc)
{
...
int CGLShaderProgram::getAttribLocation(const char* chrAttribName)
{
	VARPROP var;
...
bool CGLShaderProgram::getAttributeVar(const char* chrAttribName, VARPROP& outAttribProp)
{
	if(m_vAttribs.size() == 0)
		return false;
	for(size_t i=0; i<m_vAttribs.size(); i++)
...
bool CGLShaderProgram::getUniformVar(const char* chrUniformName, VARPROP& outUniformProp)
{
	if(m_vUniforms.size() == 0)
		return false;
	for(size_t i=0; i<m_vUniforms.size(); i++)
...
*************************************************************************/
bool CGLShaderProgram::IsGLExtensionSupported(const char *extension)
{
...
******************************************************************************/
bool CGLShaderProgram::saveBinaryProgram(const char* Filename, GLuint &ProgramObjectID)
{
#if !defined (TARGET_OS_IPHONE)
...
******************************************************************************/
bool CGLShaderProgram::loadBinaryProgram(const char* Filename, GLuint &ProgramObjectID)
{
#if !defined (TARGET_OS_IPHONE)
    // Open the file.
