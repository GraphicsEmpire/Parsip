...
#include "PS_Font/src/FTGL/ftgl.h"
				...
				for(size_t i=0; i<ctSelected; i++)
				{
					...
					if(node && node->getLock().acquire())
					{
						...
						{				
							...
							//Send Message
							actNetSendCommand(cmdMove, node, toVec4f(m_uiTransform.translate));
						}
						else if(m_uiTransform.type == uitScale)
						{	
							...
							//Send Message
							actNetSendCommand(cmdScale, node, toVec4f(m_uiTransform.scale));
						}
						else if(m_uiTransform.type == uitRotate)
						{				
...
{	
	...
	list<pair<CBlobTree*, QStandardItem *>> stkProcessing;
	...
	pair <CBlobTree*, QStandardItem*> mypair;
	QString strTag;
	...
	if(blobParent != NULL)
	{
		...
		childItem->setData((int)blobParent);
