...
namespace PS{
	...
	int CMpu::subDivide_MoveMidPointToSurface( vec3f& m, vec3f& outputNormal, vec4f& outputColor, float target_field /*= ISO_VALUE*/, int nIterations /*= DEFAULT_ITERATIONS */ )
	{
		...
		outputColor = m_root->baseColor(m);

		return (i+2)*4;				
	}

	void CMpu::compactBlobTree_Intersect(const COctree& mpuOct, CBlobTree* lpNode)
	{
	...
	int CMpu::compactBlobTree_Recursive( CBlobTree* lpInput, stack<CBlobTree*>& stkOperators, CBlobTree*& lpOutClone)
	{
		CBlobTree* clonned = lpOutClone;
		if(lpInput->isOperator())
		{			
			if(lpInput->getID() != CROSS_NONE)
			{
				clonned = cloneNode(lpInput);
				if(lpOutClone == NULL)
					lpOutClone = clonned;
				else
	...
	{		
		...
		//Process BlobTree		
		CBlobTree* reducedRoot = aLayer->getBlob();
