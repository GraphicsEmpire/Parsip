...
bool CBlobTreeToActions::convert( const char* chrOutFileName /*= NULL*/ )
{
	...
	m_Layer->convertToBinaryTree();	
	CBlobTree* root = m_Layer->getBlob();
...
//////////////////////////////////////////////////////////////////////////
bool CBlobTreeToActions::appendCommand( SKETCHCMD command, CBlobTree* node, vec4f param)
{
	...
	if(node)
	{
		...
		if(command == cmdAdd)
		{
			...
			txMsg.primType = reinterpret_cast<PS::BLOBTREE::CSkeletonPrimitive*>(node)->getSkeleton()->getType();
...
int CBlobTreeToActions::recursive_nodeToAction( CBlobTree* node )
{
	int res = 0;
	if(node->isOperator())
	{
		...
		appendCommand(cmdOperator, node, node->getColor());
	}
	else
	{
		appendCommand(cmdAdd, node, node->getColor());
	}
